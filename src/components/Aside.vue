<template>
  <b-container class="p-0 position-relative" fluid>
    <b-button @click.stop="showCollapse = !showCollapse"
              class="position-fixed p-2 p-sm-3 align-self-center menu d-flex" size="sm"
              variant="outline-secondary">
      <font-awesome-icon icon="bars" size="lg"></font-awesome-icon>
    </b-button>
    <b-row :class="{ active: showCollapse }" @click="blankClick" class="position-fixed"
           id="container" no-gutters ref="container">
      <b-col class="position-fixed d-flex flex-column align-items-stretch" cols="11" lg="4" md="6"
             sm="8">
        <b-navbar class="p-0 flex-fill d-flex flex-column align-items-stretch position-relative"
                  toggleable="true" type="light" variant="faded">
          <div
            ...
            <b-navbar-nav class="mt-4 font-weight-bold w-100">
              <b-nav-item :key="id" @click="scrollTo(link)" class="text-center" href="#"
                          v-for="(link, id) in links">{{ link.name }}
              </b-nav-item>
            </b-navbar-nav>
          </div>
        </b-navbar>
...
export default {
  ...
  methods: {
    blankClick(e) {
      if (e.target.isSameNode(this.$refs.container)) {
