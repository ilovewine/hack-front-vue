...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      clickHandler(event) {
        if (this.location.left && !event.target.isEqualNode(this.$refs.playButton.querySelector('path'))) {
          if (distance({x1: this.location.left, y1: this.location.top}, {x2: event.clientX, y2: event.clientY}) <=
              TOUCH_AREA) {
